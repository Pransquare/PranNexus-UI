    ...
    <Box component="div" sx={{ p: 2 }}>
      ...
      {title && (
        <Typography
          variant="h6"
          gutterBottom
          sx={{ fontWeight: "bold", color: "rgb(77,208,225)" }}
        >
          {title}
        </Typography>
      )}
            ...
            case "number":
              return (
                <TextField
                  ...
                  name={item.name}
                  label={item.label}
                  type={item.type === "password" ? "password" : item.type}
                  variant="outlined"
                  size="small"
                  value={item.value || ""}
                  onChange={
            ...
            case "multiSelect":
              return (
                <FormControl
                  key={item.name}
                  error={!!item.error}
                  fullWidth={item.type === "multiSelect"}
                >
                  <InputLabel size="small" id={item.name}>
                    {item.label}
                  </InputLabel>
                  <Select
                    labelId={item.name}
                    ...
                    value={item.value || (item.type === "multiSelect" ? [] : "")}
                    onChange={handleChange}
                    disabled={formDisabled || item.disable}
                    readOnly={readOnly || item.readOnly}
                    onBlur={onBlur}
            ...
            case "monthAndYearSelect":
              return (
                <LocalizationProvider dateAdapter={AdapterDayjs} key={item.name}>
                  <DatePicker
                    views={
                      item.type === "monthAndYearSelect"
                        ? ["month", "year"]
                        : undefined
                    }
                    format={item.type === "monthAndYearSelect" ? "MM-YYYY" : "DD-MM-YYYY"}
                    label={item.label}
                    value={item.value || null}
                    minDate={item.minDate || undefined}
                    maxDate={item.maxDate || undefined}
            ...
            case "dateRangePicker":
              return (
                <ClickAwayListener
                  ...
                  onClickAway={() => setShowDateRange(false)}
                >
                  <FormControl component="span" sx={{ position: "relative" }}>
                    <TextField
                      size="small"
                      label={item.label}
                      name={item.name}
          ...
          {buttonsHide.reset && (
            <Grid item>
              <Button
                ...
                size="small"
                variant="contained"
                sx={{ bgcolor: "rgb(15, 168, 233)", "&:hover": { bgcolor: "rgb(15, 168, 233)" } }}
              >
                Reset
              </Button>
            </Grid>
