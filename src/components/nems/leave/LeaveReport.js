...
function LeaveReport() {
  ...
  const roleNames = {
    hr_tools_nems_leave_manager_report: UserManagentCheck(
      "hr_tools_nems_leave_manager_report",
      roles
    ),
  ...
  const configureFormData = useCallback(
    (input) => {
      ...
      } else if (roleNames.hr_tools_nems_leave_manager_report) {
        temp.push({
          label: "Employee",
          name: "employeeId",
          type: "dropDownList",
  ...
  const submitClicked = async () => {
    ...
    try {
      ...
      const isManager = roleNames?.hr_tools_nems_leave_leaveApproval;
