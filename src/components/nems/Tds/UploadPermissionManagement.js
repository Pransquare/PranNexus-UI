...
import { enableProofDeclarationForAllActiveEmployees, enableProofDeclarationForEmployee } from "../../../service/api/nemsService/TdsService";

const UploadPermissionManagement = () => {
  ...
  const handleDialogConfirm = async () => {
    if (dialogAction === "individual" && selectedEmployee) {
      try {
        ...
        const response = await enableProofDeclarationForEmployee(selectedEmployee.employeeCode);
        if (response.success) {
      ...
      try {
        ...
        const response = await enableProofDeclarationForAllActiveEmployees();
        if (response.success) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", gap: 2 }}>
        ...
        >
          Upload Permission for Individual
        </Button>
        <Button
          variant="contained"
          color="secondary"
...
export default UploadPermissionManagement;
