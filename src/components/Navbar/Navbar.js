...
const Navbar = ({ user, toggleSidebar }) => {
  ...
  const fetchEmployeeDetails = useCallback(async () => {
    try {
      const code = localStorage.getItem("userId");
      if (!code) return;
      const response = await GetEmployeeByEmployeeCode(code);
      if (response) {
  ...
  const onloadData = async () => {
    try {
      const data = await Promise.all([GetAllDesignations(), GetGroups(), GetAllWorkLocation()]);
      setApiData({ designations: data[0], groups: data[1], workLocation: data[2] });
    } catch (error) {
      console.error("Error fetching data: ", error);
    }
  };

  const hasRole = (roleName) => userManagementData?.roleNames?.includes(roleName);
  ...
  return (
    <div className="navbar border-b-2">
      <div className="logo-container">
        <IconButton onClick={toggleSidebar}>
          <MenuSharp sx={{ color: "rgb(15,168,233)" }} />
        </IconButton>
        <img
          src={logo}
          alt="Company Logo"
      ...
      <div className="navbar-content">
        ...
        <IconButton onClick={() => navigate("/home")}>
          <HomeOutlinedIcon sx={{ fontSize: 30, color: "rgb(15,168,233)" }} />
        </IconButton>
        ...
        {/* Avatar with icon inside */}
        <div className="avatar-container">
          <IconButton onClick={handleClick} size="small">
            <Avatar sx={{ bgcolor: "rgb(15,168,233)" }}>
              <PersonOutlineIcon sx={{ color: "#fff" }} />
            </Avatar>
          </IconButton>

          <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
            {hasRole("hr_tools_nems_profile") && !employeeData?.genericProfile && (
              <MenuItem onClick={() => handleMenuItemClick("Profile Content")}>
                Profile
              </MenuItem>
            )}
            <MenuItem onClick={() => handleMenuItemClick("Reset Password")}>Reset Password</MenuItem>
            <MenuItem
              onClick={() => {
                setEmployeeData(null);
                window.localStorage.clear();
      ...
      {modalContent === "Profile Content" && (
        <ContentDailog
          ...
          content={
            <Box>
              <EmployeeDetails inputData={employeeData} apiData={apiData} readOnly disableAllFields />
            </Box>
          }
          okText={null}
          cancelText="Close"
