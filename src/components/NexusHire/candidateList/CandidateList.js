    ...
    try {
      ...
      const filteredCandidates = canView
        ? sortedCandidates.filter((candidate) => {
            ...
            if (candidate.workflowStatus === "400") return false;
            if (budget && candidate.workflowStatus === "100") return true;
            if (management && candidate.workflowStatus === "101") return true;
            return false;
          })
        : sortedCandidates;
  ...
  const contentConfig = useCallback(
    (input) => {
      return {
        ...
        content: input.map((candidate) => [
          { forAction: false, isPrint: true, value: candidate.requirementId },
          {
            isPrint: true,
            forAction: false,
            value: candidate.requirementDesc || "",
          ...
          { forAction: false, isPrint: true, value: candidate.primarySkill },
          { forAction: false, isPrint: true, value: candidate.expectedCtc },
          {
            forAction: false,
            isPrint: true,
            value: getStatusDescription(candidate.status) || "",
          ...
          { forAction: true, isPrint: false, value: { ...candidate } },
        ]),
      };
    },
    [statusMaster, workflowStatusMaster]
      ...
      {loading ? (
        ...
        <ConfigTable
          ...
          headers={headers.map((h) => (
            <span style={{ color: "rgb(15,168,233)", fontWeight: "bold" }}>
              {h}
            </span>
          ))}
          actions={actionClick}
